...
public class PriceExtractor {
    ...
    /**
     * 距离/面积单位：数字后紧跟这些单位时不算价格。
     * 覆盖真实误判案例：id 51 "2km内天街银泰" 曾被误抓成 2000 元。
     */
    private static final Pattern NON_PRICE_UNIT = Pattern.compile(
            "\\d+(\\.\\d+)?\\s*(km|KM|米|m²|㎡|平方|平米|平方米|公顷|亩)(?!.*(?:元|块))");
    ...
    private Double extractRentPrice(String text) {
        ...
        for (CandidatePrice c : candidates) {
            ...
            if (isAdjacentToNonPriceUnit(text, c.start, c.end)) {
                continue;
            }
            // 合法租金价格
            return c.value;
     ...
     */
    private boolean isAdjacentToNonPriceUnit(String text, int start, int end) {
        // 检查候选数字后面是否紧跟距离/面积单位
        String after = text.substring(end, Math.min(text.length(), end + 10));
        if (after.matches("^(?:\\s*)(km|KM|米|m²|㎡|平方|平米|平方米).*")) {
            return true;
        }
        // 检查候选数字前面是否紧跟距离关键词（如 "2km" 中数字在前）
        // 这种情况已经在 RENT_PRICE_PATTERN 中排除了（要求元/月等后缀）
        // 但需要额外检查：数字后面是 "km" 但被其他 pattern 误匹配的情况
        String before = text.substring(Math.max(0, start - 5), start);
        // 检查 "数字km" 模式——数字直接跟 km，中间无空格
        if (end < text.length()) {
            String immediate = text.substring(end, Math.min(text.length(), end + 3));
            if (immediate.matches("^(km|KM|米|㎡|m²).*")) {
                return true;
            }
        }
        return false;
